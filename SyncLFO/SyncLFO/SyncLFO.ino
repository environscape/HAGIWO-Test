...
#include <avr/io.h>
#include "const.h"  //wavetable  saw1 saw2 sine tri squ
...
// 提高pwm_freq的值，这里设置为200000，你可以根据需要进一步调整
...
void setup() {
  ...
  // 设置TCCR1A寄存器
  // 这里的设置选择了快速PWM模式，具体的位设置可能需要根据你的需求调整
  TCCR1A = 0b00100011;

  // 设置TCCR1B寄存器
  // 0b00000001表示不分频，即PWM频率 = 系统时钟频率 / OCR1A
  // 0b00000010表示分频比为8，即PWM频率 = 系统时钟频率 / 8 / OCR1A
  // 0b00000100表示分频比为64，即PWM频率 = 系统时钟频率 / 64 / OCR1B
  // 0b00001000表示分频比为256，即PWM频率 = 系统时钟频率 / 256 / OCR1B
  // 0b00010000表示分频比为1024，即PWM频率 = 系统时钟频率 / 1024 / OCR1B
  TCCR1B = 0b00000001;

  // 根据新的pwm_freq计算OCR1A的值
  OCR1A = (unsigned int)(8000000 / pwm_freq);
}
// 其他代码保持不变

// 注意：如果还需要进一步调整，可能需要修改OCR1A和OCR1B的计算方式
// 目前OCR1A的计算方式是 8000000 / pwm_freq，根据新的pwm_freq值可能需要调整
// 例如：
// OCR1A = (unsigned int)(8000000 / pwm_freq);
